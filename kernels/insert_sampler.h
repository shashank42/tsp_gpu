...
insertionStep(unsigned int *city_one, unsigned int *city_two,
              coordinates *__restrict__ location,
              unsigned int *__restrict__ salesman_route, float *__restrict__ T,
              volatile int *global_flag, unsigned int *__restrict__ N,
              curandState_t *states, float *sample_area) {
  // first, refresh the route, this time we have to change city_one-city_two
  // elements
  ...
  // From:
  // http://stackoverflow.com/questions/18501081/generating-random-number-within-cuda-kernel-in-a-varying-range
  float myrandf = curand_uniform(&states[tid]);
  myrandf *= ((float)(N[0] - 2) - 4.0 + 0.9999999999999999);
  myrandf += 4.0;
  int city_one_swap = (int)truncf(myrandf);

  city_one_swap = (city_one_swap > N[0] - 2) * (N[0] - 2) +
                  !(city_one_swap > N[0] - 2) * city_one_swap;
  city_one_swap =
      (city_one_swap < 4) * 4 + !(city_one_swap < 4) * city_one_swap;
  // Trying out normally distributed swap step
  int city_two_swap =
      (int)(city_one_swap + (curand_normal(&states[tid]) * sample_space));

  // One is added here so that if we have city two == N, then it bumps it up to
  // 1
  city_two_swap -=
      (city_two_swap >= N[0]) * ((city_two_swap / N[0]) * N[0] + 1);
  // this is N[0] - 2 because we cannot have a value of N[0] - 1,
  // So 0 + N[0] -> N[0] - 1, normally, but doing -2 gives N[0] - 2
  city_two_swap +=
      (city_two_swap <= 0) * ((-city_two_swap / N[0] + 1) * N[0] - 2);

  // Check it ||city_two - city one|| < 9, if so bump it up one
  // city_two_swap = ((city_one_swap - city_two_swap) * (city_one_swap -
  // city_two_swap) < 9) * (city_one_swap - 3) + !((city_one_swap -
  // city_two_swap) * (city_one_swap - city_two_swap) < 9) * city_two_swap;

  city_two_swap = (city_two_swap > N[0] - 2) * (N[0] - 2) +
                  !(city_two_swap > N[0] - 2) * city_two_swap;
  city_two_swap =
      (city_two_swap < 1) * 1 + !(city_two_swap < 1) * city_two_swap;
  ...
  /*
          ...
          myrandf *= ((float)max_city_two - (float)min_city_two +
     0.999999999999999); myrandf += min_city_two; int city_two_swap =
     (int)truncf(myrandf);

  */

  if (city_two_swap != (N[0] - 1) && city_two_swap != city_one_swap &&
      city_two_swap != city_one_swap - 1) {
    ...
    That means we only have to make three calculations to compute each loss
    funciton original:
    - city one - 1 -> city one
    - city one -> city one + 1
    - City two -> city two + 1
    ...
    */
    ...
    // and here I pick the latter one  because if I pick the small one, I have to
    // tell whether the flag is 0
...
__global__ static void insertionUpdateTrip(unsigned int *salesman_route,
...
__global__ static void insertionUpdate(unsigned int *__restrict__ city_one,
  ...
  /*
    1. Save city one
    2. Shift everything between city one and city two up or down, depending on
    city one < city two
...
__global__ static void insertionUpdateEndPoints(
    unsigned int *__restrict__ city_one, unsigned int *__restrict__ city_two,
    unsigned int *salesman_route, unsigned int *salesman_route2,
    volatile int *global_flag) {
  // each thread is a position in the salesman's trip
  const int xid = blockIdx.x * blockDim.x + threadIdx.x;
  /*
