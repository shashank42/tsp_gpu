...
twoOptStep(unsigned int *city_one, unsigned int *city_two,
  ...
  city_one_swap = (city_one_swap > N[0] - 1) * (N[0] - 1) +
                  !(city_one_swap > N[0] - 1) * city_one_swap;
  city_one_swap =
  ...
  */
  if (global_flag[0] != -1) {
    ...
    if (xid > city_one_swap && xid <= city_two_swap) {
      salesman_route[xid] =
          salesman_route2[city_one_swap + city_two_swap + 1 - xid];
    }
  }
}
