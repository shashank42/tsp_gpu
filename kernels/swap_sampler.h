...
__global__ static void swapStep(unsigned int *city_one, unsigned int *city_two,
  ...
  city_one_swap =
      (city_one_swap < 1) * 1 + !(city_one_swap < 1) * city_one_swap;
  ...
  city_two_swap +=
      (city_two_swap <= 0) * ((-city_two_swap / N[0] + 1) * N[0] - 1);

  // Check it ||city_two - city one|| < 9, if so bump it up one

  city_two_swap = (city_two_swap > N[0] - 1) * (N[0] - 1) +
                  !(city_two_swap > N[0] - 1) * city_two_swap;
  city_two_swap =
      (city_two_swap < 2) * 2 + !(city_two_swap < 2) * city_two_swap;

  city_two_swap =
      ((city_one_swap - city_two_swap) * (city_one_swap - city_two_swap) < 6) *
          (city_one_swap + 2) +
      !((city_one_swap - city_two_swap) * (city_one_swap - city_two_swap) < 6) *
          city_two_swap;
  ...
  // We will always have 4 calculations for original distance and the proposed
  // distance so we just unroll the loop here
  // TODO: It may be nice to make vars for the locations as well so this does
  // not look so gross The first city, unswapped. The one behind it and the one
  // in front of it
  original_dist +=
  ...
  original_dist +=
      sqrtf((location[trip_city_two_pre].x - location[trip_city_two].x) *
  ...
  proposal_dist +=
      sqrtf((location[trip_city_two_pre].x - location[trip_city_one].x) *
  ...
  proposal_dist +=
      sqrtf((location[trip_city_one_pre].x - location[trip_city_two].x) *
                (location[trip_city_one_pre].x - location[trip_city_two].x) +
            (location[trip_city_one_pre].y - location[trip_city_two].y) *
                (location[trip_city_one_pre].y - location[trip_city_two].y));
  proposal_dist +=
      sqrtf((location[trip_city_one_post].x - location[trip_city_two].x) *
                (location[trip_city_one_post].x - location[trip_city_two].x) +
            (location[trip_city_one_post].y - location[trip_city_two].y) *
                (location[trip_city_one_post].y - location[trip_city_two].y));

  // picking the first accepted and picking the last accepted is equivalent, and
  // here I pick the latter one  because if I pick the small one, I have to tell
  // whether the flag is -1
...
__global__ static void swapUpdate(unsigned int *__restrict__ city_one,
  ...
  if (tid == 0) {
    if (global_flag[0] != -1) {
      salesman_route[city_one[global_flag[0]]] =
          salesman_route2[city_two[global_flag[0]]];
      salesman_route[city_two[global_flag[0]]] =
          salesman_route2[city_one[global_flag[0]]];
    }
  }
}
