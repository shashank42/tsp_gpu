...
__global__ static void globalSwap(unsigned int* city_one,
    ...
    int sample_space = (int)floor(exp(- (T[1] / 20) / T[0]) * (float)N[0] + 8);
    ...
    while (global_flag[0] == -1 && iter < 4){
        ...
        if ((city_one_swap - city_two_swap) * (city_one_swap - city_two_swap) < 16)
...
__global__ static void localSwap(unsigned int* city_one,
	unsigned int* city_two,	coordinates* __restrict__ location,
	unsigned int* __restrict__ salesman_route,
	float* __restrict__ T,
	volatile int *global_flag,
	unsigned int* __restrict__ N,
    ...
    // This gives us a nice curve
    //http://www.wolframalpha.com/input/?i=e%5E(-+10%2Ft)+from+10+to+1
    int sample_space = (int)floor(exp(- (T[1] * 2) / T[0]) * (float)N[0] + 8);
    // Run until either global flag is zero and we do 100 iterations is false.
