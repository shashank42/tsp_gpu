 ...
 *
 * Based on:
 *https://github.com/bishma-stornelli/CVRP-AOC/blob/master/TSP-TEST.V0.9/instance.c
 *******************************/
void read_tsp(const char *tsp_file_name) {
  ...
  if (fscanf(tsp_file, "%s", buf) <= 0)
    printf("Reading file failed at first line\n");
  while (strcmp("NODE_COORD_SECTION", buf) != 0) {
    // printf("%s \n",buf);
    if (strcmp("NAME", buf) == 0) {
      if (fscanf(tsp_file, "%s", buf) <= 0)
        printf("Reading file failed at Name\n");
      // TRACE ( printf("%s ", buf); )
      if (fscanf(tsp_file, "%s", buf) <= 0)
        printf("Reading file failed at Name\n");
      strcpy(name, buf);
      ...
      if (!fgets(buf, LINE_BUF_LEN, tsp_file))
        printf("Reading failed at Comment\n");
      strcpy(comment, buf);
      ...
      if (fscanf(tsp_file, "%s", buf) <= 0)
        printf("Reading file failed at Type\n");
      // TRACE ( printf("%s ", buf); )
      if (fscanf(tsp_file, "%s", buf) <= 0)
        printf("Reading file failed at Type\n");
      // TRACE ( printf("%s\n", buf); )
      ...
      if (fscanf(tsp_file, "%s", buf) <= 0)
        printf("Reading file failed at Dim\n");
      strcpy(dim, buf);
      ...
      if (!fgets(buf, LINE_BUF_LEN, tsp_file))
        printf("Reading failed at Display Data Type\n");
      // TRACE ( printf("%s", buf); );
      buf[0] = 0;
    } else if (strcmp("DISPLAY_DATA_TYPE:", buf) == 0) {
      if (!fgets(buf, LINE_BUF_LEN, tsp_file))
      ...
      if (!fgets(buf, LINE_BUF_LEN, tsp_file))
        printf("Reading failed at Edge Weight Type\n");
    ...
    buf[0] = 0;
    if (fscanf(tsp_file, "%s", buf) <= 0)
      printf("Reading file failed during read\n");
  }
  ...
  for (int i = 0; i < meta->dim; i++) {
    if (fscanf(tsp_file, "%d %f %f", &location[i].id, &location[i].x,
               &location[i].y) < 0)
...
void read_trip(const char *trip_file_name, unsigned int *salesman_route, int N){
    ...
    if (trip_file == NULL)
        exit(EXIT_FAILURE);
    if (fscanf(trip_file,"%s", buf) <= 0) printf("Reading file failed at first
line\n"); for (int i = 0; i < N + 1; i++) if(fscanf(trip_file,"%d %f %f",
&location[i].id, &location[i].x, &location[i].y) <=0) printf("Reading failed
while scanning coordinates\n"); fclose(trip_file);
}
*/
