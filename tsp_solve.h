...
__global__ static void tspSwap_f(unsigned int* city_one,
                           unsigned int* city_two,
                           coordinates* __restrict__ location,
                           unsigned int* __restrict__ salesman_route,
                           float* __restrict__ T,
...
__global__ static void tspSwap_b(unsigned int* city_one,
	unsigned int* city_two,
	coordinates* __restrict__ location,
	unsigned int* __restrict__ salesman_route,
	float* __restrict__ T,
...
__global__ static void tspSwap2(unsigned int* city_one,
	unsigned int* city_two,
	coordinates* __restrict__ location,
	unsigned int* __restrict__ salesman_route,
	float* __restrict__ T,
	...
	while (iter < 5){
		...
		// This gives us a nice curve
		//http://www.wolframalpha.com/input/?i=e%5E(-+.2%2Ft)+from+0+to+1
		int sample_space = (int)floor(3+exp(-0.005 / T[0]) * (float)N[0]);
		// We need to set the min and max of the second city swap
		int min_city_two = city_one_swap + 3;
...
- states [curandState_t(GRID_SIZE)]
 > The seeds for each proposal steps random sample
*/
__global__ static void tspInsertion_f(unsigned int* city_one,
                           unsigned int* city_two,
                           coordinates* __restrict__ location,
                           unsigned int* __restrict__ salesman_route,
                           float* __restrict__ T,
    ...
    // This is the maximum we can sample from
    int sample_space = (int)floor(2+exp(- 0.01 / T[0]) * N[0]);
    // We need to set the min and max of the second city swap
	int min_city_two = city_one_swap + 2;
...
__global__ static void tspInsertion_b(unsigned int* city_one,
	unsigned int* city_two,
	coordinates* __restrict__ location,
	unsigned int* __restrict__ salesman_route,
	float* __restrict__ T,
	...
	// This is the maximum we can sample from
	int sample_space = (int)floor(3 + exp(-0.01 / T[0]) * N[0]);
	// We need to set the min and max of the second city swap
	int max_city_two = city_one_swap - 3;
...
__global__ static void tspInsertion2(unsigned int* city_one,
	unsigned int* city_two,
	coordinates* __restrict__ location,
	unsigned int* __restrict__ salesman_route,
	float* __restrict__ T,
	...
	// From: http://stackoverflow.com/questions/18501081/generating-random-number-within-cuda-kernel-in-a-varying-range
	float myrandf = curand_uniform(&states[tid]);
	myrandf *= ((float)(N[0] - 4) - 3.0 + 0.9999999999999999);
	myrandf += 3.0;
	int city_one_swap = (int)truncf(myrandf);
	...
	// This is the maximum we can sample from
	int sample_space = (int)floor(3+exp(-0.005 / T[0]) * N[0]);
	// We need to set the min and max of the second city swap
